...
<template>
    <div class="nft" v-for="(item,index) in new_news_data" :key="index" >
      ...
      <div class="article-item article-item--list " data-post-id="1568802" style="border-radius: 10px; background-color:#1F1D2B">
                ...
                <a  class="article-label" target="_blank" ga-bind="click" ga-category="MainPage_List" ga-action="ToItemArticle" ga-label="Article"  @click="Enter_category(item.fields.post_category)">{{ item.fields.post_category }}</a>
                <a  ga-bind="click" ga-category="MainPage_List" ga-action="ToItemArticle" ga-label="Article" class="article-link cover-block" target="_blank" :style="{'background-image':'url('+item.fields.post_cover_image+')'}" @click="Enter_content(item.fields.post_id,item.fields.post_category)"></a>
            </div>
        
        <div class="article-info js-transform">
            <h3>
                <a class="js-title-transform"  ga-bind="click" ga-category="MainPage_List" ga-action="ToItemArticle" ga-label="Article" target="_blank" @click="Enter_content(item.fields.post_id,item.fields.post_category)">{{item.fields.post_title}}</a>
            </h3>
            ...
            <div class="article-meta">
                        ...
                        <span  style="color: #bdbdbd; font-size: 15px;">{{ item.fields.created_at}}</span>
