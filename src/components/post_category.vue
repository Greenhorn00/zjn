...
const get_new_news=async(post_category:string) => {

    // console.log(page,size)

 const url:string='/news/'+post_category+'/'
...
get_new_news(post_category as string)
...
<template>
    ...
    <div class="c-archive-header__container">
      <h1 class="c-archive-header__title">{{ new_news_data[0].fields.original_category }}</h1>
    ...
    <div class="nft" v-for="(item,index) in new_news_data" :key="index"  >
      ...
      <div class="article-item article-item--list " data-post-id="1568802" style="border-radius: 10px; background-color:#1F1D2B">
                ...
                <a  class="article-label" target="_blank" ga-bind="click" ga-category="MainPage_List" ga-action="ToItemArticle" ga-label="Article">{{ item.fields.original_category }}</a>
