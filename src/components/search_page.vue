...
// //   因为切换页签和增加 查看更多 调用同一个函数，所以监听到father_page_Message变化时，让请求page=1
//   page.value=1
//   get_new_news()
// });
...
// 调用接口返回信息
// 页数和一页数据量
let new_num_flag=ref<number>(0)
let old_num_flag=ref<number>(0)
let no_more=ref<boolean>(false)
const page=ref<number>(1)
const size=ref<number>(15)

const get_new_news=async(search:string) => {

    // console.log(page,size)

 const url:string='/news/search/'
...
//判断点击加载更多后是否有新的数据被获取到
new_num_flag=new_news_data.value.length
if(new_num_flag!=old_num_flag) {
old_num_flag=new_num_flag
}
else{
    btn.value=false
    loading.value = false
...
<template>
    ...
    <div class="c-archive-header__container">
      <h1 class="c-archive-header__title">为您呈现搜索内容</h1>
    ...
    <div class="nft" v-for="(item,index) in new_news_data" :key="index"  >
      ...
      <div class="article-item article-item--list " data-post-id="1568802" style="border-radius: 10px; background-color:#1F1D2B">
                ...
                <a  class="article-label" target="_blank" ga-bind="click" ga-category="MainPage_List" ga-action="ToItemArticle" ga-label="Article" @click="Enter_category(item.fields.original_category)">{{ item.fields.post_category }}</a>
                <a  ga-bind="click" ga-category="MainPage_List" ga-action="ToItemArticle" ga-label="Article" class="article-link cover-block" target="_blank" @click="Enter_content(item.fields.post_id,item.fields.post_category)" :style="{'background-image':'url('+item.fields.post_cover_image+')'}"></a>
            </div>
        
        <div class="article-info js-transform">
            <h3>
                <a class="js-title-transform"  ga-bind="click" ga-category="MainPage_List" ga-action="ToItemArticle" ga-label="Article"  @click="Enter_content(item.fields.post_id,item.fields.post_category)" target="_blank">{{item.fields.post_title}}</a>
            </h3>
...
<button class="discover-loadbtn" @click="load()" style="color:#bdbdbd; text-align: center; margin-left;: 50% " v-if="btn " >查看更多</button>
<p v-if="loading" style="color:#bdbdbd; font-size: 48px; text-align:center; margin-top: 10%;">玩命加载中...</p>
<p v-if="no_more" style="color:#bdbdbd; font-size: 48px; text-align:center; margin-top: 10%;">客官，没有更多喽...</p>
...
.c-archive-header__container {
    margin-top: 32px;
    margin-bottom: 24px;
    padding-left: 24px;
    border-left: 4px solid #fadc23;
}   
</style>